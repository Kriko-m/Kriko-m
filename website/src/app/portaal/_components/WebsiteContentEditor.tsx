...
const inputStyle = { width: '100%', padding: '8px 11px', border: '1.5px solid #CBD5E1', borderRadius: 8, fontFamily: 'inherit', fontSize: '.88rem', color: '#162544', boxSizing: 'border-box' as const }
const labelStyle = { display: 'block', fontSize: '.74rem', fontWeight: 800, color: '#162544', marginBottom: 4, textTransform: 'uppercase' as const, letterSpacing: '.04em' }
      ...
      <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, pointerEvents: 'none', padding: 16 }}>
        <div style={{ pointerEvents: 'auto', width: '96%', maxWidth: 620, maxHeight: '92vh', background: '#fff', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.22)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ margin: 0, color: '#162544', fontWeight: 900, fontSize: '1.2rem', fontFamily: 'var(--font-heading, Nunito, sans-serif)' }}>Website-content</h3>
              <p style={{ margin: '2px 0 0', color: '#64748B', fontSize: '.82rem' }}>Pas de leiding-contactgegevens op de publieke takpagina’s aan.</p>
            </div>
            <button onClick={onClose} type="button" style={{ width: 34, height: 34, border: '1.5px solid #CBD5E1', borderRadius: 8, background: 'none', color: '#64748B', fontSize: '1.1rem', cursor: 'pointer' }}>✕</button>
          </div>
            ...
            {PORTAAL_TAKKEN.map(tak => (
              <button
                ...
                style={{
                  ...
                  border: activeTak === tak ? `2px solid ${TAK_KLEUREN[tak]}` : '1.5px solid #CBD5E1',
                  background: activeTak === tak ? '#EBF0F9' : '#fff', color: activeTak === tak ? '#162544' : '#555555',
                }}
              >
                {TAK_NAMEN[tak]}
              </button>
          ...
          <div style={{ overflow: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {flash && <div style={{ background: '#EBF0F9', border: '1.5px solid #CBD5E1', color: '#162544', padding: '8px 12px', borderRadius: 8, fontSize: '.82rem', fontWeight: 700 }}>{flash}</div>}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {f.leaders.length === 0 && <p style={{ color: '#64748B', fontSize: '.82rem', margin: 0 }}>Nog geen leiding toegevoegd.</p>}
                {f.leaders.map((l, i) => (
                ...
                <button onClick={() => addLeader(activeTak)} type="button" style={{ alignSelf: 'flex-start', marginTop: 2, padding: '6px 14px', background: 'none', border: '1.5px dashed #CBD5E1', borderRadius: 8, color: '#64748B', fontSize: '.82rem', fontWeight: 700, cursor: 'pointer' }}>+ leiding toevoegen</button>
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: '14px 24px', borderTop: '1px solid #E2E8F0', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button onClick={onClose} type="button" style={{ padding: '9px 16px', background: 'none', border: '1.5px solid #CBD5E1', borderRadius: 8, fontWeight: 700, color: '#475569', cursor: 'pointer', fontSize: '.88rem' }}>Sluiten</button>
            <button onClick={save} disabled={saving} type="button" style={{ padding: '9px 24px', background: '#162544', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 900, cursor: 'pointer', fontSize: '.88rem' }}>{saving ? 'Opslaan…' : 'Alles opslaan'}</button>
          </div>
        </div>
      </div>
    </>
