 ...
 * falls back to PHP mail(). Bewaart niets lokaal (geen e-maillog).
 *
...
function scouts_send_mail($to, $subject, $body_html, $from = 'no-reply@kriko-m.be') {
    ...
    // GEEN lokale e-maillog meer: we bewaren geen e-mailinhoud of -adressen
    // op de server (privacy). De mail wordt enkel verzonden.

    // Try SMTP via socket on localhost:1025 (Mailpit default)
    $host = '127.0.0.1';
    $port = 1025;
